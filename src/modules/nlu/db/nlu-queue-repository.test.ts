...
import { NLUQueueRepository } from './nlu-queue-repository';
import { RoundRepository } from '../../scheduling/db/round-repository';
import { GroupRepository } from '../../group/db/group-repository';

const prisma = new PrismaClient();
const nluQueueRepo = new NLUQueueRepository();
const roundRepo = new RoundRepository();
const groupRepo = new GroupRepository();

describe('NLUQueueRepository - OpenCode API Failure Recovery (Story 4.5)', () => {
  let group: any;
  let round: any;
  ...
  beforeEach(async () => {
    ...
    group = await groupRepo.findOrCreate('retry-group', 'Retry Group');
    round = await roundRepo.create(group.id, 'Retry Meeting', 'tomorrow');
  });
  ...
  test('should queue a new pending NLU request', async () => {
    ...
    const request = await nluQueueRepo.queue(round.id, userId, rawResponse, error);
  ...
  test('should increment retry count and update nextRetryAt on retry', async () => {
    ...
    // Initial queue
    const initialRequest = await nluQueueRepo.queue(round.id, userId, rawResponse);
    const initialId = initialRequest.id;

    // First retry (retryCount = 1)
    const updatedRequest = await nluQueueRepo.updateRetry(initialId, 1, 'Still down');
  ...
  test('should mark request as completed', async () => {
    ...
    const request = await nluQueueRepo.queue(round.id, userId, 'Free Wednesday');
    
    const completed = await nluQueueRepo.markCompleted(request.id);
    expect(completed.status).toBe('completed');
  });

  test('should mark request as failed after max retries', async () => {
    const userId = 'user-retry-4';
    const request = await nluQueueRepo.queue(round.id, userId, 'Free Thursday');
    
    const failed = await nluQueueRepo.updateRetry(request.id, 5, 'Max retries reached');
  ...
  test('should retrieve pending requests for retry', async () => {
    ...
    const pending = await nluQueueRepo.findPendingForRetry();
